/**
 * 37_EthPoseWeb — Ethernet MJPEG + ESP-DL COCO-17 pose (YOLO11n-Pose)
 *
 * Models on the chosen volume:
 *   /models/p4/coco_pose_yolo11n_pose_s8_v2.espdl   (default)
 *   /models/p4/coco_pose_yolo11n_pose_s8_v1.espdl   (optional)
 *
 * Copy from library models/espdl/p4/ or upload via WebFileManager :82.
 * Ports: :80 Camera/Pose UI · :82 WebFileManager · :83 WFM transfers
...
static const int kVals[] = {(int)ESP32P4_POSE_YOLO11N_V2, (int)ESP32P4_POSE_YOLO11N_V1};
static const char *kLabs[] = {"YOLO11n-Pose v2", "YOLO11n-Pose v1"};
...
static esp32p4_pose_t g_poses[8];

static void rebuildVfsPaths() { store.vfsPath(kSettings, sizeof(kSettings), kSettingsFs); }

static int modelFromUi(int m) {
  return (m == (int)ESP32P4_POSE_YOLO11N_V1) ? ESP32P4_POSE_YOLO11N_V1
                                                 : ESP32P4_POSE_YOLO11N_V2;
}

static const char *modelFile(int m) {
  return (m == (int)ESP32P4_POSE_YOLO11N_V1)
             ? "/models/p4/coco_pose_yolo11n_pose_s8_v1.espdl"
             : "/models/p4/coco_pose_yolo11n_pose_s8_v2.espdl";
}
...
static void detHook(uint16_t *rgb, int w, int h, void *) {
  ...
  if (ui.model_req) {
    ...
    pose.end();
    det_ready = false;
    if (modelsPresent(ui.model) && pose.begin(modelFromUi(ui.model))) {
      pose.setScoreThr((float)ui.thr_pct / 100.f);
      det_ready = true;
    } else {
      Serial.printf("pose: model %d missing on %s\n", ui.model, store.label());
    }
  }
  if (ui.thr_req) {
    ui.thr_req = false;
    if (pose.ready()) pose.setScoreThr((float)ui.thr_pct / 100.f);
  }
  if (ui.settings_dirty) {
    ui.settings_dirty = false;
    saveSettings();
  }
  if (!ui.detect_en || !pose.ready()) {
    ui.objs = 0;
    ui.ms = 0;
    ui.summary[0] = '\0';
    return;
  }
  int n = pose.detect(rgb, w, h, g_poses, 8);
  ui.objs = n;
  ui.ms = pose.lastMs();
  snprintf(ui.summary, sizeof(ui.summary), "person x%d", n);
  if (n > 0) pose.draw(rgb, w, h, g_poses, n);
}

void onEthEvent(arduino_event_id_t event) {
  switch (event) {
    case ARDUINO_EVENT_ETH_START:
      ETH.setHostname("esp32p4-pose");
      break;
...
static bool startEthAndWfm() {
  ...
  if (!wfm) return false;
  wfm->setName("Pose models").setPorts(WFM_UI_PORT, WFM_FILE_PORT).setHomePort(CAM_UI_PORT);
  if (!wfm->begin()) {
...
static void printUploadHelp(const IPAddress &ip) {
  Serial.println();
  Serial.printf("Missing pose .espdl on %s — upload via WebFileManager:\n", store.label());
  Serial.printf("  Files  http://%s:%u/\n", ip.toString().c_str(), (unsigned)WFM_UI_PORT);
  Serial.printf("  Put under /%s/models/p4/  (coco_pose_yolo11n_pose_s8_v2.espdl)\n", store.label());
...
static bool startCameraDet() {
  ...
  auto &ui = stream.detUi();
  if (!pose.begin(modelFromUi(ui.model))) {
    Serial.printf("Pose begin FAILED — camera UI still up, upload via Files :%u\n",
  ...
  stream.setDetCatalog("Pose", "COCO pose", "YOLO11n-Pose · 17 keypoints · /models/p4/", kN, kVals,
                       kLabs);
  stream.setFilesBrowserPort(WFM_UI_PORT);
  stream.setFrameHook(detHook, nullptr);
  loadSettings();
  if (det_ready) {
    if (ui.model != (int)pose.model()) {
      pose.end();
      det_ready = false;
      if (modelsPresent(ui.model) && pose.begin(modelFromUi(ui.model))) {
        pose.setScoreThr((float)ui.thr_pct / 100.f);
  ...
  Serial.printf("Files  http://%s:%u/\n", ip.toString().c_str(), (unsigned)WFM_UI_PORT);
  Serial.printf("Pose ready=%d model=%d thr=%d%%\n", pose.ready() ? 1 : 0, (int)pose.model(),
                ui.thr_pct);
  return true;
}
...
void setup() {
  Serial.begin(115200);
  delay(1200);
  Serial.println("=== 37_EthPoseWeb (ESP-DL COCO pose) ===");
...
void loop() {
  ...
  if (millis() - last >= 2000) {
    last = millis();
    auto &ui = stream.detUi();
    Serial.printf("[hb] poses=%d ms=%d det=%d model=%d\n", ui.objs, ui.ms, ui.detect_en ? 1 : 0,
                  ui.model);
  }
}
