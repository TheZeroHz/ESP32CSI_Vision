#include <ESP32CSI_Vision.h>

// 16x16 solid red JPEG generated for decoder smoke-test.
static const uint8_t kTinyJpg[] = {
    };

ESP32P4_Jpeg jpeg;

void setup() {
  Serial.begin(115200);
  delay(1200);
  Serial.println("=== 03_JpegDecode ===");
  if (!jpeg.begin(64, 64, 50)) {
    Serial.println("jpeg engine FAILED");
    return;
  }
  uint32_t w = 0, h = 0;
  if (!jpeg.decodeInfo(kTinyJpg, sizeof(kTinyJpg), &w, &h)) {
    Serial.println("header parse FAILED");
    return;
  }
  Serial.printf("header %ux%u\n", (unsigned)w, (unsigned)h);
  uint8_t *rgb = (uint8_t *)esp32p4_psram_alloc(64 * 64 * 2);
  size_t n = jpeg.decode(kTinyJpg, sizeof(kTinyJpg), rgb, 64 * 64 * 2, &w, &h);
  Serial.printf("decoded %u bytes  %ux%u\n", (unsigned)n, (unsigned)w, (unsigned)h);
}

void loop() { delay(1000); }
