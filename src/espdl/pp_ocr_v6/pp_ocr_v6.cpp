    ...
    m_model(nullptr), m_image_preprocessor(nullptr), m_postprocessor(nullptr), m_letterbox_img({}), m_resize_scale(1.0f)
{
    ...
    auto sd_path = std::filesystem::path(esp32p4_model_mount_point()) / CONFIG_PP_OCR_V6_MODEL_SDCARD_DIR / model_name;
...
#endif
    if (!m_model || m_model->get_inputs().empty()) {
        ESP_LOGE(TAG, "failed to load det model %s", model_name);
        delete m_model;
        m_model = nullptr;
        return;
    }
...
#endif
    if (!m_model || m_model->get_inputs().empty()) {
        ESP_LOGE(TAG, "failed to load rec model %s", model_name);
        delete m_model;
        m_model = nullptr;
        return;
    }
