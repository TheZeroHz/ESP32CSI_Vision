...
Yolo11nSeg::Yolo11nSeg(const char *model_name, float score_thr, float nms_thr)
{
    ...
    m_model = new dl::Model(path,
                            ...
                            param_copy);
#else
    auto sd_path = std::filesystem::path(esp32p4_model_mount_point()) / CONFIG_COCO_SEG_MODEL_SDCARD_DIR / model_name;
    m_model = new dl::Model(sd_path.c_str(), fbs::MODEL_LOCATION_IN_SDCARD);
#endif
    dl::TensorBase *in = m_model ? m_model->get_input() : nullptr;
    if (!in || !in->data) {
        ESP_LOGE("coco_seg", "failed to load %s", model_name);
        delete m_model;
        m_model = nullptr;
        return;
    }
