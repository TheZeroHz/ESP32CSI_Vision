...
esp_err_t DataBase::enroll_feat(TensorBase *feat)
{
    ...
    if (!feat_copy) {
        ESP_LOGE(TAG, "PSRAM alloc failed for feature.");
        return ESP_FAIL;
    }
    ...
    size_t size = fwrite(&m_meta, sizeof(database_meta), 1, f);
    if (size != 1) {
        ESP_LOGE(TAG, "Failed to write database meta.");
        // roll back memory
        heap_caps_free(m_feats.back().feat);
        m_feats.pop_back();
        m_meta.num_feats_total--;
        m_meta.num_feats_valid--;
        fclose(f);
