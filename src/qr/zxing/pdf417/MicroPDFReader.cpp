...
// No Log.h in Arduino-vendored tree (avoid #define log — clashes with std::log)
#define ZX_MPDF_LOG(...) ((void)0)
...
static auto ToInts(const FixedPattern<6, 10> in[52])
{
...
static const auto LRRAPInts = ToInts(LRRAPs);
static const auto CRAPInts = ToInts(CRAPs);
...
static Clusters FindCandidates(const BitMatrix& image, bool tryHarder, bool reversed)
{
	...
	for (int y = margin; y < height - margin; y += skip) {
		...
		(std::tie(next, idx) = FindLRAP(next), next.isValid()) {
			...
			ZX_MPDF_LOG(centered(PointF(reversed ? width - 1 - p.x : p.x, reversed ? height - 1 - p.y : p.y)), 1);
			...
			if (p != PointI{}) {
				...
				if (tryHarder) {
					...
					if (rowHeight < skip) {
						...
						skip = rowHeight;
						ZX_MPDF_LOG("decreasing skip to %d at x=%3d, y=%3d, p.width=%d", skip, p.x, y, p.width);
						break;
					} else if (rowHeight > 2 * skip
							   && std::all_of(res.begin(), res.end(), [&](const Cluster& c) { return p.y - c.back().y > 2 * skip; })) {
						skip = std::min(8, rowHeight); // reset skip to default when starting a new cluster
						ZX_MPDF_LOG("increasing skip to %d at x=%3d, y=%3d, p.width=%d", skip, p.x, y, p.width);
					}
				}
#endif
	...
	res.remove_if([&](Cluster& lraps) {
		...
		int M = Size(std::max_element(segs.begin(), segs.end(), [](const auto& a, const auto& b) { return Size(a.lraps) < Size(b.lraps); })->lraps);
		...
		if (Size(segs) < 3 || std::abs(segs.back().idx - segs.front().idx) > 2 * Size(segs)
			|| (LRAP_WITH_CW && !std::is_sorted(segs.begin(), segs.end(), [](const Segment& a, const Segment& b) { return a.idx < b.idx; })))
			return true;
		...
		lraps.erase(std::transform(segs.begin(), segs.end(), lraps.begin(), center), lraps.end());
	...
	for (const auto& cluster : res) {
		for (auto lrap : cluster)
			ZX_MPDF_LOG("%d @ %dx%d (width: %d)", lrap.idx, lrap.x, lrap.y, lrap.width);
		ZX_MPDF_LOG();
	}
#endif
	return res;
...
{
	ZX_MPDF_LOG("right: %s, ms: %.1f", ToString(start.d).c_str(), start.ms);
	std::array<int, 16> colHist = {}, offsets = {};
	for (int s = 0; s < 2 && *std::max_element(colHist.begin(), colHist.end()) < 3; ++s)
		for (auto& p : lraps) {
			auto cur = start;
			auto tmp = cur;
			...
			auto checkRAP = [&](RAP rap, int colI) {
				...
				ZX_MPDF_LOG("%2d/%2d: %2d %s ", pair.first, pair.second, pair.offset, pair.isValid() ? "<  " : "   ");
				return pair.isValid();
			};
	...
	ZX_MPDF_LOG("colHist: ", "%2d ", colHist);
	ZX_MPDF_LOG("offsets: ", "%2d ", offsets);
	int nCol = int(std::max_element(colHist.begin(), colHist.end()) - colHist.begin());

	if (colHist[nCol]) {
		auto offset = double(offsets[nCol]) / colHist[nCol];
		start.d = bresenhamDirection((10. + 17. * 2) * start.d - 2. * offset * start.right());
		ZX_MPDF_LOG("average offset: %.1f, new right: %s, ms: %.1f", offset, ToString(start.d).c_str(), start.ms);
	}
...
{
	auto rotFamMax = std::max_element(rotFamHist.begin(), rotFamHist.end());
	...
	float meanCount = Reduce(cwMat, 0.f, [](float acc, const Codeword& e) { return acc + e.count; })
					  / std::count_if(cwMat.begin(), cwMat.end(), [](const auto& e) { return e.count > 0; });
	...
	for (const auto& s : SYMBOLS) {
		...
		ZX_MPDF_LOG("symbol: %d, %2dx%2d, rotFam: %2d, firstRow: %2d, error: %d",
...
static Barcode ScanCandidate(const BitMatrix& image, const Cluster& lraps)
{
	...
	ZX_MPDF_LOG("down: %s", ToString(down).c_str());
	...
	auto checkRAP = [&](int li, int ri) {
		...
		if (!rap.isValid())
			return false;
		// ZX_MPDF_LOG("li: %2d, ri: %2d, ri-li: %2d, fam: %d, offset: %d", li, ri, rap.second - rap.first, rap.family, rap.offset);
	...
	for (; image.isIn(startCur.p + down) && (inSweepRange(startCur.p) || failedTries < 10 * startCur.ms); startCur.p += down, failedTries++) {
		...
		// ZX_MPDF_LOG("li: %2d @ (%f, %f)", li, cur.p.x, cur.p.y);
		if (!li)
			continue;
		startCur.ms = cur.ms;
		Codeword cw[4];
		...
		ZX_MPDF_LOG("%2d/%d -> ", li, RAPCluster(li));
		for (int x = 0; x < nCols; ++x) {
			ZX_MPDF_LOG("%3d/%d ", cw[x].codeword, cw[x].cluster);
			if (cw[x]) {
				li += rowOffset(cw[x].cluster);
				if (li < 1 || li > 52)
					continue;
				auto& cell = histMat(x, li);
				auto cwPtr = std::find(cell.begin(), cell.end(), cw[x]);
				if (cwPtr != cell.end()) {
					cwPtr->count++;
	...
	std::transform(histMat.begin(), histMat.end(), cwMat.begin(), [](std::vector<Codeword>& hist) {
		if (hist.empty())
			return Codeword{};
		if (hist.size() == 1)
			return hist.front();

		auto best = std::max_element(hist.begin(), hist.end(), [](const auto& a, const auto& b) { return a.count < b.count; });
		bool bestIsUnique = std::all_of(hist.begin(), hist.end(), [&](const auto& a) { return &a == &*best || a.count < best->count; });
	...
	for (int y = 0; y < cwMat.height(); ++y) {
		...
		for (int x = 0; x < cwMat.width(); ++x) {
			auto& e = cwMat(x, y);
			e.count ? ZX_MPDF_LOG("%3d %2d | ", e.codeword, e.count) : ZX_MPDF_LOG("       | ");
		}
		ZX_MPDF_LOG();
	}
#endif
	...
	ZX_MPDF_LOG("size: %dx%d, firstRow: %d, cws: %d, rotFamHist: %d/%d/%d/%d, rotFam: %d, nEECs: %d, erasures: %d, valid: %d", si.nCols,
	...
	return Barcode(std::move(decoderResult), DetectorResult({}, std::move(pos)), BarcodeFormat::MicroPDF417);
}
...
#ifdef PRINT_DEBUG
		// LogMatrixWriter unavailable without Log.h
#endif
