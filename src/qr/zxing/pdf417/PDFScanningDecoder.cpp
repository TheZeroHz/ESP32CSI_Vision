...
DecoderResult DecodeCodewords(std::vector<int>& codewords, int numECCodewords, const std::vector<int>& erasures)
{
	if (codewords.empty())
		return FormatError();
	...
	int minCodewordWidth, int maxCodewordWidth)
{
	...
	// static_cast: ESP32 builds with -fno-rtti; PDF417 always attaches DecoderResultExtra.
	auto meta = static_cast<DecoderResultExtra*>(res.extra().get());
	if (meta)
