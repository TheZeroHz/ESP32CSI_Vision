...
template <int LEN, int RET_LEN = LEN - 2>
std::array<int, RET_LEN> NormalizedE2EPattern(const PatternView& view, int mods, bool reverse = false)
{
...
std::array<int, RET_LEN> NormalizedE2EPattern(const PatternView& view)
{
	return NormalizedE2EPattern<LEN, RET_LEN>(view, SUM);
}

/** Nominal-module FixedPattern → E2E (moduleSize == 1). Enables constexpr RAP tables. */
template <int LEN, int SUM, int RET_LEN = LEN - 2, bool IS_SPARCE>
constexpr std::array<int, RET_LEN> NormalizedE2EPattern(const FixedPattern<LEN, SUM, IS_SPARCE>& pattern)
{
	std::array<int, RET_LEN> e2e{};
	for (int i = 0; i < RET_LEN; i++)
		e2e[i] = pattern[i] + pattern[i + 1];
	return e2e;
}
